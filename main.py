...
The search_by_budget tool returns product names with fabric: "base configuration".

**Format budget search results exactly as the tool specifies!**

If the tool returns:
```
{
  "products": [
    {"name": "Midhurst 3 Seater Sofa", "fabric": "base configuration", "base_price": 1937}
  ]
}
```

WRONG ❌:
"Midhurst 3 Seater Sofa in Premium Velvet Navy - £1,937"

CORRECT ✅:
"Midhurst 3 Seater Sofa - £1,937 (base configuration)"

**NEVER invent specific fabric names for budget searches!** Base prices are for entry-level fabrics and vary by product.

ALWAYS add this note: "Prices shown are base for standard configurations—exact price depends on fabric choice."
    ...
    try:
        ...
        for product_name, product_data in PRODUCT_SKU_MAP.items():
            ...
            if base_price <= max_price:
                # Extract product name from full_name (ignore fabric as it's often wrong in products.json)
                # Example: "Alwinton 3 Seater Sofa in RHS Botanicals Plantae Japonicae Teal"
                full_name = product_data.get("full_name", product_name)
                product_name_only = full_name.split(" in ")[0] if " in " in full_name else full_name

                matching_products.append({
                    "name": product_name_only,
                    "fabric": "base configuration",  # Base prices are for standard/entry-level fabrics
        ...
        # Debug: Print first product to verify fabric field is included
        if matching_products:
            print(f"  [DEBUG] First product: {matching_products[0]}")
        ...
        print(f"  [DEBUG] Returning {len(matching_products)} products to Grok")
        import json
...
def find_best_matches(query, mapping, fuzziness=85):
    ...
    for keyword, value in mapping.items():
        ...
        # Prefix match: check if query contains a prefix of the keyword
        # Example: query "3 seater" should match keyword "3 seater sofa"
        else:
            keyword_words = keyword.split()
            # Try progressively shorter prefixes (longest first for best match)
            for i in range(len(keyword_words), 0, -1):
                keyword_prefix = ' '.join(keyword_words[:i])
                # Skip single-word prefixes that are too short (avoid false matches)
                if len(keyword_prefix) < 3:
                    continue
                if re.search(r'\b' + re.escape(keyword_prefix) + r'\b', query, re.IGNORECASE):
                    # Lower confidence for prefix matches, proportional to prefix length
                    confidence = 80 + len(keyword_prefix)
                    matches.append((keyword, value, confidence))
